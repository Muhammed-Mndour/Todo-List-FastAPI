...
def generate_custom_exception_handler(status_code: int, *,
    ...
    def err_handler(request: Request, exception: Exception) -> JSONResponse:
        ...
        error_message = (exception.message if hasattr(exception,
                                                      "message") and exception.message else None) or client_error_msg
        ...
        if include_traceback:
            error_details['traceback'] = traceback.format_exception(type(exception), exception,
...
def register_error_handlers(app):
    ...
    err_handler_req_validation = generate_custom_exception_handler(status_code=400, client_error_message=str,
                                                                   include_traceback=app.debug)
    err_handler_integrity = generate_custom_exception_handler(status_code=400, client_error_message=str,
                                                              include_traceback=app.debug)
    err_handler_assertion = generate_custom_exception_handler(status_code=400, client_error_message=str,
                                                              include_traceback=app.debug)
    err_handler_http = generate_custom_exception_handler(status_code=400, client_error_message=lambda exc: exc.detail,
                                                         include_traceback=app.debug)
    err_handler_500 = generate_custom_exception_handler(status_code=500,
